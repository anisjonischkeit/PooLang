...
import           Network.URI                    ( uriAuthority
                                                , uriRegName
                                                )
...
rewritePage page localSite foreignSite = BL8.pack newText
  where
    replacedHostname = replace foreignSite localSite (BL8.unpack page)
    replacedLogo     = replace
        "<img src=\"/images/logos/php-logo.svg\" width=\"48\" height=\"24\" alt=\"php\">"
        "PooLang"
        replacedHostname
    newText = replace "PHP" "PooLang" replacedLogo
...
fullPath request =
    BL8.fromStrict $ (Wai.rawPathInfo request) <> (Wai.rawQueryString request)
...
rerouteResponse originalRequest localSite foreignSite = do
    ...
    request <-
        ...
        $  "http://"
        ++ foreignSite
        ++ (BL8.unpack $ fullPath originalRequest)
    response <- httpLbs request manager

    return $ Wai.responseLBS
        status200
        ([]) -- This should be replaced with some of php's response headers (ie. Content-Type) 
        (rewritePage (NC.responseBody response) localSite foreignSite)
...
app request respond = do
    ...
    response         <- case (maybeLocalSite, maybeForeignSite) of
        (Just localSite, Just foreignSite) ->
            rerouteResponse request localSite foreignSite
